	...
	<div class="page-body" style="">
       ...
       <div class="box">
            ...
            <div class="box-body">
              <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
               ...
               <div class="row"><div class="col-sm-12"><table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                ...
                <tr role="row"><th class="sorting_asc" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 100px;">SlNo.</th>
                  ...
                  <th class="sorting_asc" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="">Parent Category</th>
                  <th class="sorting_asc" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="">Image</th>
                ...
                <tbody>
                  @if(count($subcategories)>0)
  	                ...
  	                <td><span>{{ (($subcategories->currentPage() - 1 ) * $subcategories->perPage() ) + $loop->iteration }}</span></td>
  	                  <td class="sorting_1"><span>{{$subcategory->name}}</span></td>
                      <td class="sorting_1"><span>{{$subcategory['category']['name']}}</span></td>
                      <td class="sorting_1"><span><img src="{{asset('categories/'.$subcategory->image)}}" style="width: 70px;height: 70px"></span></td>
                      @php ($checked = '')
                      @if($subcategory->status==0)
                      @php ($checked = 'checked')
                      @endif
                      <td><input type="checkbox" {{$checked}} value="{{$subcategory->id}}" class="status" data-toggle="toggle" data-on="Active" data-off="Inactive">
                      </td>
  	                  <td><a href="{{ url('subcategory/edit/'.$subcategory->id) }}"><i style="font-size:24px;margin-left: 20px;" class="fa">&#xf044;</i></a></td>
  	                  <td><a href="#" data-id="{{$subcategory->id}}" class="delete"><span><i class="fa fa-trash" aria-hidden="true" style="font-size:24px;margin-left: 30px;"></i></span></a></td>
  	                </tr>
  	                @endforeach
                    @else
                    <tr>
              ...
              </table></div></div>{{ $subcategories->links() }}</div>
            </div>
            <!-- /.box-body -->
          </div>
		          <!-- /.box -->
    ...
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Sub Category delete</h4>
        </div>
        <div class="modal-body">
          <p>Do you want delete this Sub category..?</p>
        </div>
        <div class="modal-footer">
          {{ Form::open(['route' => 'subcategory.delete','method'=>'post']) }}
          {!! Form::hidden('category_id','', ['class' => 'form-control category_id']) !!}
...
$('.status').on('change', function(e)
{
   ...
   if($(this).prop("checked") == true)
   {
    ...
    $.ajaxSetup({
     ...
     $.ajax({
      url:"{{ route('change-sub-category-status') }}",
      method:"POST",
      data : {category_id:category_id,cur_status:cur_status},
