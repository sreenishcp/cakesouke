	...
	<div class="page-body" style="">
       ...
       <div class="box">
            ...
            <div class="box-body">
               ...
               <div class="row"><div class="col-sm-12"><table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                ...
                <tbody>
                  @if(count($categories)>0)
  	                ...
  	                <td><span>{{ (($categories->currentPage() - 1 ) * $categories->perPage() ) + $loop->iteration }}</span></td>
  	                  <td class="sorting_1"><span>{{$category->name}}</span></td>
                      
                      <td class="sorting_1"><span><img src="{{asset('categories/'.$category->image)}}" style="width: 70px;height: 70px"></span></td>
                      @php ($checked = '')
                      @if($category->status==0)
                      @php ($checked = 'checked')
                      @endif
                      <td><input type="checkbox" {{$checked}} value="{{$category->id}}" class="status" data-toggle="toggle" data-on="Active" data-off="Inactive">
                      </td>
  	                  <td><a href="{{ url('category/edit/'.$category->id) }}"><i style="font-size:24px;margin-left: 20px;" class="fa">&#xf044;</i></a></td>
  	                  <td><a href="#" data-id="{{$category->id}}" class="delete"><span><i class="fa fa-trash" aria-hidden="true" style="font-size:24px;margin-left: 30px;"></i></span></a></td>
  	                </tr>
  	                @endforeach
                    @else
                    <tr>
              ...
              </table></div></div>{{ $categories->links() }}</div>
            </div>
            <!-- /.box-body -->
          </div>
		          <!-- /.box -->
    ...
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          {{ Form::open(['route' => 'category.delete','method'=>'post']) }}
